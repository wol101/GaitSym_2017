...
#ifdef _IRR_WCHAR_FILESYSTEM // nasty hack to stop wide characters in the OBJ file
    io::path name;
    core::cutFilenameExtension(name,file->getFileName()) += ".mtl";
    file->write("# exported by Irrlicht\n",23);
    file->write("mtllib ",7);
    char *wcstombs_buf = new char[name.size() * 2 + 1];
    memset(wcstombs_buf, 0, name.size() * 2 + 1);
    wcstombs(wcstombs_buf, name.c_str(), name.size() * 2 + 1);
    file->write(wcstombs_buf, strlen(wcstombs_buf));
    delete [] wcstombs_buf;
    file->write("\n\n",2);
#else
    io::path name;
...
#ifdef _IRR_WCHAR_FILESYSTEM // nasty hack to stop wide characters in the OBJ file
                char *wcstombs_tbuf = new char[tname.size() * 2 + 1];
                memset(wcstombs_tbuf, 0, tname.size() * 2 + 1);
                wcstombs(wcstombs_tbuf, tname.c_str(), tname.size() * 2 + 1);
                file->write(wcstombs_tbuf, strlen(wcstombs_tbuf));
                delete [] wcstombs_tbuf;
                file->write("\n",1);
#else
