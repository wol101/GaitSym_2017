    ...
    InputReceiverEnabled(true), TargetAndRotationAreBound(false)
{
    ...
    recalculateViewArea();
    // wis - added in the option of using a right handed camera
    IsRHCoordinates = false;
    // ~wis
...
// wis - added in the option of using a right handed camera
//! Gets the RH status of the camera.
//! \return Field of view of the camera
bool CCameraSceneNode::isRHCoordinates() const
{
    return IsRHCoordinates;
}

//! Sets the RH status of the camera (true RH coordinate system, false LH coordinate system).
/** \param isRHCoordinates: control flag. */
void CCameraSceneNode::setRHCoordinates(bool isRHCoordinates)
{
    IsRHCoordinates = isRHCoordinates;
}
// ~wis

// wis - added in the option of setting the IsOrthogonal variable
//! Sets the IsOrthogonal status of the camera (true orthogonal projection, false perspective projection).
/** \param isOrthogonal: control flag. */
void CCameraSceneNode::setOrthogonal(bool isOrthogonal)
{
    IsOrthogonal = isOrthogonal;
}
// ~wis
...
{
    // wis - added in the option of using a right handed camera and orthographic projections
    ...
    {
        irr::f32 widthOfViewVolume = ((ZNear + ZFar) / 2) * sin(Fovy);
        irr::f32 heightOfViewVolume = widthOfViewVolume / Aspect;
        if (IsRHCoordinates == false)
            ViewArea.getTransform ( video::ETS_PROJECTION ).buildProjectionMatrixOrthoLH(widthOfViewVolume, heightOfViewVolume, ZNear, ZFar);
        else
