...
public:
    ...
    void SetBodyQueryData(const std::map<std::string, Body *> &bodyMap);
...
public:
    ...
    int DoSearch(const std::map<std::string, Body *> &bodyMap);
    ...
    WarehouseUnit *GetWarehouseUnit(unsigned int index) { if (index >= m_warehouseList.size()) return 0; return m_warehouseList[index]; }
